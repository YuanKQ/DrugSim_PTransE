...
public:
    ...
    void run()
    {
        ...
        double predict_ddi_num = 0;
        double predict_sim_num = 0;
        double a_len_one = 0;
        double predict_other_num = 0;
        /*END*/
		...
		for (int testid = 0; testid<fb_l.size()/2; testid+=1)
		{
			...
			for (int i=a.size()-1; i>=0; i--) {
                if (a.capacity()>=2) a_len_one += 1;
		cout<<"size: "<<a.size()<<"["<<i<<"]";
                cout<<"a first: "<<a[i].first<<" second:"<<a[i].second<<endl;
			...
			if (testid%100==0)
			{
                   ...
                   for (int i=a.size()-1; i>=0; i--)
                        cout<<"["<<i<<"] "<<a[i].first<<" "<<a[i].second<<endl<<"-------------"<<endl;
        ...
        cout<<"testid: "<<testid<<" predict_sim_num: "<<predict_sim_num<<" predict_ddi_num: "<<predict_ddi_num<<" predict_other_num: "<<predict_other_num<<endl;
	...
	//	cout<<"right:"<<rsum/fb_r.size()<<'\t'<<rp_n/fb_r.size()<<'\t'<<rsum_filter/fb_r.size()<<'\t'<<rp_n_filter/fb_r.size()<<endl;
        ...
        cout<<"true_ddi:"<<predict_true_ddi_num<<"\ttotal_ddi:"<<total_ddi_num<<"\tpredict_ddi"<<predict_ddi_num<<endl;
        cout<<"precision:"<< predict_true_ddi_num/total_ddi_num<<"\trecall:"<<predict_true_ddi_num/predict_ddi_num<<endl;
        cout<<"fb_l.size:"<<fb_l.size()<<" a_len_one: "<<a_len_one<<" predict_sim_num: "<<predict_sim_num<<" predict_ddi_num: "<<predict_ddi_num<<" predict_other_num: "<<predict_other_num<<endl;
    }
};
Test test;
